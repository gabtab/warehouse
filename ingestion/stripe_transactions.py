...
def fetch_transactions_from_stripe():
    transactions = []
    last_id = None
    while True:
        if last_id:
            current_page = stripe.BalanceTransaction.list(limit=100, starting_after=last_id)
        else:
            current_page = stripe.BalanceTransaction.list(limit=100)
        if len(current_page.data) == 0:
            break
        transactions.extend(current_page.data)
...
stripe_sessions = fetch_transactions_from_stripe()

# Convert Stripe data to DataFrame
df = convert_to_dataframe(stripe_sessions)

# Load data into PostgreSQL
load_data_into_postgres(df, 'stripe_transactions','landing_python')
